...
void Game::run()
{
    ...
    while (run_)
    {
        ...
        {
            Timer::delayMicrosecs(static_cast<unsigned long>(1000000 /
                FRAMES_PER_SECOND - frameTimer_.getTicks()));
        }
    }
}
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case SDL_KEYDOWN:
            ...
            if ((event.key.keysym.sym == SDLK_F4 && event.key.keysym.mod &
                 KMOD_ALT)
                    or (event.key.keysym.sym == SDLK_w  && event.key.keysym.mod
                        & KMOD_CTRL))
            {
                run_ = false;
                break;
